...
insurance1 <- insurance %>% select(-Claim) %>%  mutate_if(is.factor, as.numeric)
```
...
kinsurance_n<-as.data.frame(lapply(insurance1,normalize))
```
...
```{r}
#Mengambil Label
kinsurance_train_target<-insurance[index_train,8]
kinsurance_test_target<-insurance[-index_train, 8]

barplot(table(kinsurance_train_target), col=c("red","blue"))
barplot(table(kinsurance_test_target), col=c("red","blue"))
```
